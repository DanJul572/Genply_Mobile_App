...
const Recomended = ({ firstOpened, refreshing, navigation, changeRequestStatus }) => {
    ...
    const getGames = async () => {
        changeRequestStatus('recomended', true);
        setGames([]);
        let user_id = await AsyncStorage.getItem('user_id');
        let url = BASE_URL + '/api/rekomendasigame/' + user_id + '/' + DEVICE;
        let headers = {
        ...
        axios.get(url, { headers })
            .then(response => {
                ...
                changeRequestStatus('recomended', false);
            })
            .catch(error => {
                changeRequestStatus('recomended', false);
    ...
    return (
        <View>
            <Text style={styles.title}>{t('recomended_text')}</Text>
            {(games.length > 0) && (
                <FlatList
                    data={games}
                    renderItem={({ item }) =>
