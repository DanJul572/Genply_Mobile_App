...
const LastLocalLeaderboard = ({ firstOpened, refreshing, changeRequestStatus }) => {
    ...
    const getPlayers = async () => {
        changeRequestStatus('last_local_leaderboard', true);
        setPlayers([]);
        let generate_month = 9 - 1;
        let month = 0;
        ...
        .then(response => {
            ...
            changeRequestStatus('last_local_leaderboard', false);
        })
        .catch(error => {
            changeRequestStatus('last_local_leaderboard', false);
    ...
    return (
        <View style={styles.container}>
            <ImageBackground source={ LeaderboardWinnerImage } style={styles.background}>
                <Text style={styles.text}>{t('last_local_player_text')}</Text>
...
export default LastLocalLeaderboard
